...
import { usersList } from "../utils/credential.js";
...
export const createProductModal = (productData) => {
  ...
  buyFirstBtnEl.addEventListener("click", (evt) => {
    cartList.push(productData);

    rootEl.append(cartModal(cartList));
    // sto appendendo nuovamente il carrello
...
// carrello
export const cartModal = (productList) => {
  const titleWrapper = cE("h2");
  const cartWrapper = cE("div");
  const closeCart = cE("button");
  ...
  // renderizzo il carrello
  cartWrapper.classList.add("cartModal");
  titleWrapper.classList.add("titleCart");
  titleWrapper.textContent = "i tuoi prodotti";
  // buyBtn.setAttribute("type" ,"button");
  buyBtn.setAttribute("value", "buy");

  // renderizziamo come si mostra il prodotto
  productList.forEach((item) => {
    // const renderProductCart = cE("div");
    const title = cE("h2");
    // appendi
    // const description = cE("p");
    // const quantity = cE("input");
    // const total = cE("p");
    // const totalInput = cE("input");
    // const image = cE("img");
    title.textContent = item.title;
    cartWrapper.appendChild(title);
  });

  cartWrapper.append(titleWrapper, buyBtn, closeCart);

  return cartWrapper;
};

export const loginModal = () => {
  const container = cE("form");
  const name = cE("input");
  ...
  container.append(name, password, submit);
